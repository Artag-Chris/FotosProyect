...
import FileBase from 'react-file-base64';//este lo usamos para convertir imagenes a texto y guardarlos en mongo
...
const Form = ({ currentId, setCurrentId }) => {
  ...
  return (
    //aqui se envian los datos del formulario y se usa el filebase64
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editar "${post.title}"` : 'enviar una foto'}</Typography>
